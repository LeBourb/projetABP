	...
	function add_pop_up() {
		...
		if ( wc_get_page_id( 'terms' ) > 0 && apply_filters( 'woocommerce_checkout_show_terms', true ) ) { 
			...
			jQuery(document).ready(function($) {
				...
				$( window ).bind( 'updated_checkout', function() {
					...
					$(document).on('click',"#checkoutpopupform_close",function(){
						...
						history.pushState('data', '', '<?php echo wc_get_cart_url(); ?>');
			...
			<div id="checkoutpopupform" class="simplepopup"  >
				...
				<?php if($this->terms_conditions_popup_agree_enable == "yes") { ?>				
					<a id="checkoutpopupform_close" class="mypopupbuttonclass simplepopupClose btn btn-lg btn-danger" style="float:left; text-align:center; cursor:pointer; height: 10%; width: 30%;"><?php echo $pop_up_button_text; ?></a>				
				<?php }?>
				
				<?php if($this->terms_conditions_popup_close == "yes") { ?>				
					<a id="close_popup" class="mypopupbuttonclass btn btn-lg btn-default" style="float:left; text-align:center; cursor:pointer; height:10%; width:30%; border: 2px solid #333333; border-radius: 0px; color: #333;"><?php if($this->terms_conditions_popup_close_text !='') { echo $this->terms_conditions_popup_close_text; }else {echo __('Close',$DC_Checkout_Terms_Conditions_Popup->text_domain);} ?></a>				
				<?php }?>
				</div>					
			</div>
