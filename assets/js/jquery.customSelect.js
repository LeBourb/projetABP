    ...
    _create: function () {
      ...
      select.on(eventPrefix + 'focus', function () {
        if (!root.hasClass(disabledClass)) {
          ...
          setTimeout(function() {$(document).on('click.customRange' + self.rootId, function (event) {
            if ($.contains($(root[0]).find('.ui-customSelect-dropdown')[0], event.target)) {
              self._trigger('blur');
            }
          })});
        }
    ...
    _createFromSelect: function () {
      ...
      this.element.children().each(function () {
        var element = this,
            ...
            label = '<label for="' + id + '" class="' + element.attributes.class.value + '">' + element.innerHTML + '</label>',
            input;
        if (self.isMultiple) {
